...
export const metadata: Metadata = {
  title: 'Japanese Watch Lots | New Japan Deals',
  description:
    'Shop Japanese watch lots direct from Japan. Multiple watches in one listing — great value bundles. ' +
    'Worldwide shipping. Licensed dealer since 2014.',
  alternates: { canonical: 'https://www.newjapandeals.com/lot' },
  openGraph: {
    title: 'Japanese Watch Lots | New Japan Deals',
    description: 'Multiple Japanese watches in one listing. Great value bundles from Japan.',
    url: 'https://www.newjapandeals.com/lot',
    type: 'website',
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Japanese Watch Lots | New Japan Deals',
    description: 'Watch lot bundles from Japan. Ships worldwide.',
  },
};
...
interface Product {
  ...
  is_lot?: boolean; lot_quantity?: number;
}

async function fetchLotProducts(): Promise<Product[]> {
  try {
    const res = await fetch(
      `${API_URL}/api/products.php?is_lot=1&status=published&limit=100`,
      { next: { revalidate: 300 } },
    );
...
async function LotGrid() {
  const products = await fetchLotProducts();
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">No watch lots yet</h2>
          <p className="text-gray-500 mb-6">Check back soon — new lots added regularly.</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-6">
          <nav className="text-sm text-gray-500 mb-3">
            ...
            <span className="text-gray-800">Watch Lots</span>
          </nav>
          <h1 className="text-3xl font-bold text-[#1A1A1A]" style={{ fontFamily: 'Playfair Display, serif' }}>
            Japanese Watch Lots
          </h1>
          <p className="text-gray-500 mt-1">{products.length} lot listing{products.length !== 1 ? 's' : ''} available</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-5">
          {products.map((p, idx) => {
            ...
            const qty = p.lot_quantity && p.lot_quantity > 1 ? p.lot_quantity : null;
            return (
              <article key={p.id} className={`bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow ${isSold ? 'opacity-75' : ''}`}>
                <Link href={`/product/${p.slug}`} className="block">
                  ...
                  <div className="p-4">
                    ...
                    {qty && (
                      <p className="text-xs text-purple-700 mb-1">Includes {qty} items</p>
                    )}
...
export default function LotPage() {
  const jsonLd = {
    ...
    '@type': 'CollectionPage',
    name: 'Japanese Watch Lots',
    description: 'Multiple Japanese watches bundled in one listing, direct from Japan.',
    url: `${SITE_URL}/lot`,
